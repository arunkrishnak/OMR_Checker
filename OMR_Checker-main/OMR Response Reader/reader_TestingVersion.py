...
def readeromr(path,zoom_factor=3):
    ...
    # cv2.imwrite("Trimmed_Image.jpg", img)
    ...
    imgThresh = cv2.threshold(stid, 180, 255,cv2.THRESH_BINARY_INV )[1] 
    ...
    imgThresh = cv2.threshold(sversion, 180, 255,cv2.THRESH_BINARY_INV )[1] # APPLY THRESHOLD AND INVERSE
    #cv2.imshow("Thresh",imgThresh)
